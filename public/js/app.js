 ...
 * deep dive tab switching, report export, scan history, and custom backend API configuration.
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnToggleSettings && settingsModal) {
  ...
  // Check Backend Health and load history on startup
  checkBackendHealth();
  loadHistoryRecords();

  // ==========================================
  // 2. Health & Settings Management
  // ==========================================
  async function checkBackendHealth() {
    ...
    try {
      ...
      if (res.ok) {
        if (statusTextDisplay) statusTextDisplay.textContent = "Engine Connected";
        return true;
      }
    } catch (e) {
      if (statusTextDisplay) statusTextDisplay.textContent = "Backend Offline";
      return false;
    }
    return false;
  }
  ...
  function openSettingsModal() {
    ...
    if (customBackendInput) {
      customBackendInput.value = localStorage.getItem("phishguard_backend_url") || "";
    }
  ...
  async function handleSaveSettings() {
    ...
    if (val) {
      localStorage.setItem("phishguard_backend_url", val);
    } else {
    ...
    if (settingsStatusMsg) {
      ...
      settingsStatusMsg.textContent = "Testing connection to " + (val || "relative /api") + "...";
    }

    const isHealthy = await checkBackendHealth();
    if (isHealthy) {
      if (settingsStatusMsg) {
        settingsStatusMsg.style.color = "var(--safe-color)";
        settingsStatusMsg.textContent = "✔ Successfully connected to backend API!";
      }
      setTimeout(closeSettingsModal, 1200);
      loadHistoryRecords();
    } else {
      if (settingsStatusMsg) {
        settingsStatusMsg.style.color = "var(--critical-color)";
        settingsStatusMsg.textContent = "⚠️ Could not connect to API at this URL. Please verify server is online and CORS is permitted.";
      }
    }
  }
  ...
  function handleResetSettings() {
    ...
    if (settingsStatusMsg) {
      settingsStatusMsg.style.display = "block";
      settingsStatusMsg.style.color = "var(--safe-color)";
      settingsStatusMsg.textContent = "Reset to default relative API routing.";
    }
    checkBackendHealth();
  ...
  // ==========================================
  // 3. Analysis Execution & API Communication
  // ==========================================
  ...
  async function executeAnalysis(url) {
    ...
    try {
      ...
      const currentTarget = apiBase || window.location.origin;
      showError(
        "Backend Connection Failure",
        `Unable to reach the security analysis engine at (${currentTarget}/api/analyze). ` +
        `If running on Netlify, ensure the Flask backend is active (or configured in the API settings).`
      );
    } finally {
  ...
  // ==========================================
  // 6. History Drawer & Persistence
  // ==========================================

  async function loadHistoryRecords() {
    const apiBase = getApiBase();
    try {
      const response = await fetch(`${apiBase}/api/history?limit=25`);
      const data = await response.json();
  ...
  async function handleClearHistory() {
    ...
    const apiBase = getApiBase();
    try {
      const response = await fetch(`${apiBase}/api/history`, { method: "DELETE" });
      const data = await response.json();
